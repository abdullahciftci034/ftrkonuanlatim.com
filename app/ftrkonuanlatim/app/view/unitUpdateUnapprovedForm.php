...
<div id="guncelle">
    ...
    <form  id="guncelle_form" action="<?php echo APP_ROOT1; ?>index.php" method="POST" enctype="multipart/form-data"  unitId='<?php echo @$arr["id"]; ?>' unitKey='<?php echo @$arr["unitKey"]; ?>'>
        <label for="content">İçerik</label>
        <br>
        <label for="unitTitle">Konu Başlığı</label>
        <br>
        <input type="text" name="unitTitle" value="<?php if(is_array(@$arr)){ echo @$arr["unitTitle"]; }  ?>" placeholder="Konu başlığı">
        <br>
...
<script>
    ...
    //ajax işlemlerimiz
    $(function(){
        $("form#guncelle_form").submit(function(e){
            e.preventDefault();
            var obj=$(this);
            var data=new FormData(obj[0]);
            content = $("div[name=content-icerik] div.ql-editor").html();
            data.append("method","ajax");
            data.append("page","unitUpdateUnapproved");
            data.append("unitContent",content);
            data.append("unitKey",obj.attr("unitKey"));
            data.append("unitId",obj.attr("unitId"));
            $.ajax({
                url:obj.attr("action"),
...
<style type="text/css"> 
    ...
    form#guncelle_form label{
        text-align:left;
        margin-bottom: 7px;
        font-size: 18px;
        font-weight: 500;    
    }
    form#guncelle_form textarea{
        width: 100%;
        height: 500px;
    }
    form#guncelle_form input{    
        width:100% ;
        margin-bottom:10px;
    }
    form#guncelle_form select{    
        width:100% ;
        margin-bottom:7px;
    }
    form#guncelle_form div#button{
        text-align: center;
    }
    form#guncelle_form button{
        font-weight: 700;
        width: 80%;
    ...
    form#guncelle_form input[ss="son"]{
        margin-bottom: 15px;
        font-size: 15px;
        height: 25;
    }
