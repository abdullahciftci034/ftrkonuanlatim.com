...
class createSitemap {
    ...
    public function searchLink($link){
        $data=$this->dom->getElementsByTagName('url');
        $data1=null;
        foreach($data as $val){
            $data1=$val->childNodes;
            foreach($data1 as $val1){
               if($val1->nodeValue==$link){
                    return $val;
                }    
            }
        }
    ...
    #burda ise güncelllemleri yaptık
    public function setLink($arr){
        $node=$this->searchLink($arr["link"]);
        unset($arr["link"]);
        if($node != null){
            foreach($node->childNodes as $key => $val){
                if(!empty(@$arr[@$val->tagName])){
                    $val->textContent=$arr[$val->tagName];
                }
            }    
        }else{
            return false;
